    ...
    def sync_file(self, file_path):
        ...
        try:
            cmd = [
                ...
                '--single-file', str(file_path)
            ]
            
            if self.auto_commit:
                cmd.append('--auto-commit')
            
            result = subprocess.run(cmd, capture_output=True, text=True, cwd=self.sync_script.parent.parent)
...
class ObsidianWatcher:
    def __init__(self, obsidian_vault_path, sync_script_path, hugo_content_path, auto_commit=False):
...
def main():
    parser = argparse.ArgumentParser(description='Watch Obsidian vault for changes and auto-sync to Hugo')
